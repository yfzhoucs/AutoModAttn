...
from utils.load_data_rgb_abs_action_auto_corrected_lang import DMPDatasetEERandTarXYLang, pad_collate_xy_lang
...
def train(writer, name, epoch_idx, data_loader, model, 
    ...
    for idx, (img, target, joint_angles, ee_pos, ee_traj, length, phis, mask, target_xy_center, target_xy_mask, sentence, joint_angles_traj) in enumerate(data_loader):
        ...
        phis = phis.to(device)
        mask = mask.to(device)
        attn_index_tar = pixel_position_to_attn_index(target_xy_center * 100, attn_map_offset=5)
        sentence = sentence.to(device)
        joint_angles_traj = joint_angles_traj.to(device)
        ee_traj = torch.cat((ee_traj, joint_angles_traj[:, -1:, :]), axis=1)
        target_xy_center = torch.tensor(target_xy_center, dtype=torch.float32).to(device)
        target_xy_mask = torch.tensor(target_xy_mask, dtype=torch.float32).to(device)

        # print(target_xy_center)
        ...
        if stage == 0:
            ...
            target_xy_pred, ee_pos_pred, attn_map, attn_map2, attn_map3 = model(img, joint_angles, sentence, phis, stage)
        else:
            target_xy_pred, ee_pos_pred, attn_map, attn_map2, attn_map3, attn_map4, trajectory_pred = model(img, joint_angles, sentence, phis, stage)
        ...
        # Attention Loss
        loss_attn = loss_attn_layer1 + loss_attn_layer2 + loss_target_pos_attn
        loss = 0

        if stage >= 1:
            loss0 = criterion(target_xy_pred * target_xy_mask, target_xy_center * target_xy_mask)
            ...
            print('obj1 pred', target_xy_pred[0].detach().cpu().numpy())
            print('obj1 g_t_', target_xy_center[0].detach().cpu().numpy())
            print('obj2 pred', target_xy_pred[1].detach().cpu().numpy())
            print('obj2 g_t_', target_xy_center[1].detach().cpu().numpy())
        ...
        if stage == 0:
            ...
            print(f'epoch {epoch_idx}, step {idx}, stage {stage}, l_all {loss.item():.2f}, l0 {loss0.item():.2f}, l2 {loss2.item():.2f}')
        else:
            print(f'epoch {epoch_idx}, step {idx}, stage {stage}, l_all {loss.item():.2f}, l0 {loss0.item():.2f}, l2 {loss2.item():.2f}, l4 {loss4.item():.2f}')
...
def test(writer, name, epoch_idx, data_loader, model, criterion, train_dataset_size, stage, print_attention_map=False, train_split=False):
    with torch.no_grad():
        ...
        for idx, (img, target, joint_angles, ee_pos, ee_traj, length, phis, mask, target_xy_center, target_xy_mask, sentence, joint_angles_traj) in enumerate(data_loader):
            ...
            phis = phis.to(device)
            mask = mask.to(device)
            attn_index_tar = pixel_position_to_attn_index(target_xy_center, attn_map_offset=5)
            sentence = sentence.to(device)
            joint_angles_traj = joint_angles_traj.to(device)
            ee_traj = torch.cat((ee_traj, joint_angles_traj[:, -1:, :]), axis=1)
            target_xy_center = torch.tensor(target_xy_center, dtype=torch.float32).to(device)
            ...
            if stage >= 1:
                target_xy_pred = target_xy_pred.detach().cpu()
                target_xy_center = target_xy_center.detach().cpu()
                error_target_position_this_time = torch.sum(((target_xy_pred[:, :2] - target_xy_center[:, :2]) * 100) ** 2, axis=1) ** 0.5
            ...
            if stage >= 1:
                print(idx, f'err tar pos: {(error_target_position / num_datapoints).item():.4f} err ee pos: {(error_ee_pos / num_datapoints).item():.4f}')
            if stage >= 2:
...
def main(writer, name, batch_size=96):
    ...
    # load model
    model = Backbone(img_size=224, embedding_size=192, num_traces_in=7, num_traces_out=10, num_weight_points=36, input_nc=3)
    if ckpt is not None:
    ...
    dataset_test = DMPDatasetEERandTarXYLang(val_data_dirs, random=True, length_total=36)
    ...
    data_loader_train_dmp = torch.utils.data.DataLoader(dataset_train_dmp, batch_size=batch_size,
                                          shuffle=True, num_workers=8,
                                          collate_fn=pad_collate_xy_lang)
    dataset_test_dmp = DMPDatasetEERandTarXYLang(val_data_dirs, random=False, length_total=36)
...
if __name__ == '__main__':
    name = 'train-abs-action-auto-dense-waypoints-1800-corrected-lang-no-supattn'
